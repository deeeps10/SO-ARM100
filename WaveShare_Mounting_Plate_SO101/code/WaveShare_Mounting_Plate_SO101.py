...
# 'Sketch2': 3 segments → Line/RadiusArc profile
...
with BuildSketch(_inclined_plane_2) as sk_Sketch2_2:
    with BuildLine():
        Line((202.8618, 122.5084), (185.0, 104.2685))
        Line((185.0, 104.2685), (167.1382, 122.5084))
        # Arc split: sweep=268.8deg >= 150 — emitted as two half-arcs
        RadiusArc((167.1382, 122.5084), (185.0, 165.0), 25.0)
        RadiusArc((185.0, 165.0), (202.8618, 122.5084), 25.0)
...
with BuildSketch(_inclined_plane_3) as sk_Sketch2_3:
    with BuildLine():
        # Arc split: sweep=268.8deg >= 150 — emitted as two half-arcs
        RadiusArc((202.8618, -157.4916), (185.0, -115.0), -25.0)
        RadiusArc((185.0, -115.0), (167.1382, -157.4916), -25.0)
        Line((167.1382, -157.4916), (185.0, -175.7315))
        Line((185.0, -175.7315), (202.8618, -157.4916))
...
with BuildSketch(_inclined_plane_4) as sk_Sketch2_4:
    with BuildLine():
        # Arc split: sweep=268.8deg >= 150 — emitted as two half-arcs
        RadiusArc((-167.1382, -157.4916), (-185.0, -115.0), -25.0)
        RadiusArc((-185.0, -115.0), (-202.8618, -157.4916), -25.0)
        Line((-202.8618, -157.4916), (-185.0, -175.7315))
        Line((-185.0, -175.7315), (-167.1382, -157.4916))
...
with BuildSketch(_inclined_plane_5) as sk_Sketch2_5:
    with BuildLine():
        # Arc split: sweep=268.8deg >= 150 — emitted as two half-arcs
        RadiusArc((-167.1382, 122.5084), (-185.0, 165.0), -25.0)
        RadiusArc((-185.0, 165.0), (-202.8618, 122.5084), -25.0)
        Line((-202.8618, 122.5084), (-185.0, 104.2685))
        Line((-185.0, 104.2685), (-167.1382, 122.5084))
    _inc_edges_sk_Sketch2_5 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch2_5 = Wire.combine(_inc_edges_sk_Sketch2_5)[0]
_wire_sk_Sketch2_5 = _wire_sk_Sketch2_5.moved(_inclined_plane_5.location)
_mkf_sk_Sketch2_5 = BRepBuilderAPI_MakeFace(_inclined_plane_5.wrapped, _wire_sk_Sketch2_5.wrapped, True)
_face_sk_Sketch2_5 = Face(_mkf_sk_Sketch2_5.Face())
...
_solid_sk_Sketch4_7 = extrude(_face_sk_Sketch4_7, amount=11.484, dir=Vector(0.0, 0.0, 1.0), taper=-44.0).solid()

# 'Sketch5': 4 segments → Line/RadiusArc profile
_inclined_plane_8 = Plane(
    origin=Vector(0.0, 0.0, 64.8881),
    x_dir=Vector(1.0, 0.0, 0.0),
    z_dir=Vector(0.0, 0.0, 1.0),
)
with BuildSketch(_inclined_plane_8) as sk_Sketch5_8:
    with BuildLine():
        Line((90.59, -90.09), (-90.59, -90.09))
        Line((-90.59, -90.09), (-90.59, 90.09))
        Line((-90.59, 90.09), (90.59, 90.09))
        Line((90.59, 90.09), (90.59, -90.09))
    _inc_edges_sk_Sketch5_8 = list(BuildSketch._get_context().pending_edges)
# Build inclined-plane face outside BuildSketch (bypasses BRepFill_Filling)
from OCP.BRepBuilderAPI import BRepBuilderAPI_MakeFace
_wire_sk_Sketch5_8 = Wire.combine(_inc_edges_sk_Sketch5_8)[0]
_wire_sk_Sketch5_8 = _wire_sk_Sketch5_8.moved(_inclined_plane_8.location)
_mkf_sk_Sketch5_8 = BRepBuilderAPI_MakeFace(_inclined_plane_8.wrapped, _wire_sk_Sketch5_8.wrapped, True)
_face_sk_Sketch5_8 = Face(_mkf_sk_Sketch5_8.Face())
...
with BuildPart() as part:
    ...
    _vec = Vector(0.0, 0.0, 1.0) * -75.0
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.SUBTRACT)
    # Fusion depth expression: -75.000000 mm
    ...
    # --- FEATURE: Extrude4 ---
    # -- Extrude4 --
    _face = _face_sk_Sketch4_7
    _solid = _solid_sk_Sketch4_7
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: 11.484031678 mm
    # Fusion taper angle expression: 44 deg
    
    # --- FEATURE: Extrude5 ---
    # -- Extrude5 --
    _face = _face_sk_Sketch5_8
    _vec = Vector(0.0, 0.0, 1.0) * 11.1119
    _solid = Solid.extrude(_face, _vec)
    add(_solid, mode=Mode.ADD)
    # Fusion depth expression: 11.111941337 mm
    

# -- Export --
export_step(part.part, 'WaveShare_Mounting_Plate_SO101.step')
export_stl(part.part,  'WaveShare_Mounting_Plate_SO101.stl')
